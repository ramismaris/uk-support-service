...
- создаёт обращение;
- нажимает «Написать по заявке №N» или «Ответить» под сообщением сотрудника;
- отправляет сообщение, которое попало в обращение: ответом (reply), выбором на шаге 4 или по правилам 2–3.
...
| Сообщение клиента | назначенный сотрудник (если он ещё сотрудник и не заблокирован); иначе все сотрудники | текст + «Открыть» |
| Сообщение сотрудника | клиент | «💬 Заявка №N» + текст и файлы + кнопка «Ответить»; `max_message_id` сохраняется для правила 1 маршрутизации. Сообщение сохраняется, только если Max его принял: иначе API отвечает 502 и в чате ничего не появляется |
...
Сообщение клиенту о смене статуса — «🟡 Статус заявки №N: Нужен ваш ответ…»; при отклонении с причиной, при закрытии с вопросом «Проблема решена?» («Вопрос решён?» у вопроса) и кнопками «Да»/«Нет». Оно сохраняется в чат обращения как `SYSTEM` с `max_message_id`: ответ на него (reply) попадает в это обращение по правилу 1, а панель видит, что получил клиент. Комментарий сотрудника к другим переходам остаётся в истории, клиент его не видит.
