...
│   │   ├── theme/            # оформление панели — для любого вошедшего
│   │   ├── webhooks/         # вебхук Max
...
- **Приветствие** (`services/welcome_service.py`): `/start`, «Начать» и «Главное меню» — одно сообщение: фото из `WELCOME`, текст и меню. Раздел открывается правкой только со своей клавиатурой — фото из сообщения уходит, «Главное меню» возвращает его. Фото загружается в Max при первом показе, токен хранится в `files.max_token`. Если фото недоступно (файла нет, Max не принял загрузку или отклонил сообщение с фото), приветствие уходит без фото. Токен привязан к боту: сменили бота при той же базе — загрузите фото приветствия заново.
...
- **Первые админы** из `ADMIN_MAX_USER_IDS` получают роль при входе или первом сообщении боту. Конфиг роль только повышает и возвращает её при каждом входе, поэтому такого админа в панели не понизить и не заблокировать (409). Чтобы понизить — убрать из конфига, затем понизить в админке. Себя понизить или заблокировать тоже нельзя — так в системе всегда остаётся админ.
...
Подключение — `/api/v1/ws?token=…`. Сервер принимает соединение и сразу закрывает его с кодом `4401`, если токена нет, он неизвестен или истёк, и с кодом `4403`, если это не сотрудник или он заблокирован. Сессия БД закрывается сразу после проверки токена: открытая панель не держит соединение с базой. События публикуют сервисы после коммита (`services/events.py`), отправка идёт в фоне — медленная панель не задерживает ответ API и бота. Пропущенные за время обрыва события не досылаются: после переподключения панель перечитывает список. Когда сотрудника блокируют или понижают до клиента, его открытые соединения закрываются с кодом `4403` — роль снимается сразу и для уже открытой панели.
...
- Картинки контента (фото приветствия, логотип) админ загружает через `POST /api/v1/admin/content/images`; в сидах их нет. Принимаются только JPEG и PNG, тип определяется по содержимому, а не по заголовку. В `WELCOME.file_id` и `THEME.logo_file_id` можно указать только такой файл — без заявки и сообщения: фото из заявки — персональные данные, из приветствия его увидели бы все пользователи бота.
...
Раскладка обращений: телефон — список и обращение по очереди; от `lg` — полоса навигации | список | обращение. Обращение — шапка с одной главной кнопкой статуса и меню «⋯», карточка жильца над чатом (раскрывается), чат с событиями статусов из истории. Закрытие отправляется через 5 секунд — до этого его можно отменить. Цвета — токены Max UI, акцент — `--brand` (см. `.impeccable.md`).
...
**Контент** (Б6, только админ). `GET /api/v1/admin/content` — все блоки одним ответом (`null`, если блока нет или он сломан). Правка — `PUT /api/v1/admin/content/{welcome|emergency|services|payment|contacts|theme}`, у каждого блока своя схема тела. Картинку сначала загружают (`POST /api/v1/admin/content/images`, поле `file`), её `id` указывают в `file_id` или `logo_file_id`. Ограничения: тексты — до 3000 символов, текст кнопки оплаты — до 64 (лимит Max), ссылка — только `http(s)`, цвет — `#RRGGBB`, телефонов — до 10. В ответах `file_url` и `logo_url` — подписанные ссылки, живут час. Бот читает контент на каждом апдейте: правка видна сразу.

**Оформление** — `GET /api/v1/theme` для любого вошедшего (название, цвет, `logo_url`); 404 — не настроено, панель берёт свои значения. Ссылка на логотип живёт час: открытая дольше панель перечитывает тему.

**Пользователи** (Б6, только админ). `GET /api/v1/admin/users` — список, сначала новые. Фильтры: `role` (можно несколько: `?role=MANAGER&role=ADMIN`), `is_blocked`, `q` — часть имени, ника или телефона либо точный Max id (его показывает команда боту `/id`). `PATCH /api/v1/admin/users/{id}` с телом `{"role": ..., "is_blocked": ...}`: поле не передано или `null` — не меняется. 409 — снять права админа с себя или с админа из конфига.

**Текст с символом NUL** (`\u0000`) отклоняется (422, в ответе сотрудника — 400): Postgres его не хранит. Из имени загружаемого файла NUL вырезается.
...
- **Правка сообщения с фото** (2026-09-26): правка только с клавиатурой убирает фото; правка с тем же токеном фото возвращает; сообщению без фото его можно добавить правкой.
