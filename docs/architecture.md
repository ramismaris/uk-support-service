...
- **Ответ (reply):** у входящего сообщения `link.type == "reply"`, `link.message.mid` совпадает с `mid` из ответа на отправку (`message.body.mid`). Правило 1 маршрутизации работает, если хранить `mid` сообщений бота.
